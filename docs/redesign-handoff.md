...
当前交付版本以 `origin/main` 的 HEAD 为准；交付前再次确认 `HEAD == origin/main`。
...
本轮先完成案例研究，再选定实现方案。研究记录与取舍见 [project-showcase-research.md](./project-showcase-research.md)。

- MacBook 继续作为四个同级项目的入口；项目打开后，屏幕连续扩展为 Work 区域主舞台，机身边框淡出，关闭时反向收回。
- 鼠标在屏幕内滚动项目内容，在屏幕外围滚动外层页面；不再要求用户先点中内部窗口。
- Timeline 改为一条共享年份轴上的教育 / 项目双栏，每个年份只占一个纵向坐标；Intro 同时取消整屏最小高度。
- 阿里云以双工作主线为首屏；智谱以可操作工作流为首屏；智能缺陷以 16:9 视频为首屏；Smart Ganttodo 以 1280 × 720 等比缩放的真实网页为首屏。
- 项目长 inspector 改成舞台下方的紧凑项目导航、角色、焦点与证据条。
- 本轮视觉对照、滚动验证与控制台结果见仓库根目录 [design-qa.md](../design-qa.md)。
...
本轮“研究案例后重做项目舞台”的提交位于该表之后的 `origin/main` HEAD；不为在同一提交内回填自身哈希而制造额外文档提交。
